...
import { MS_PER_DAY, type Viewport, originForAnchor, xToMs } from "@/lib/projection";
  ...
  const zoomAt = useCallback(
    (offsetX: number, deltaY: number) => {
      ...
      setState((prev) => {
        ...
        // Клампим масштаб ДО вычисления origin: на упоре в макс/мин зум якорь не «уезжает».
        const minPpd = minPxPerDay(width, bounds);
