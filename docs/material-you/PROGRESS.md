...
- **Попутно (по запросу визуальной проверки, унаследовано из master — не регрессии Ф4):** (1) тултип EventLayer у краёв больше не сужается в узкую колонку и не вылезает — `w-max max-w-56` + clamp `left` в `[TOOLTIP_HALF_PX, width−TOOLTIP_HALF_PX]`; (2) GridCanvas годовой LOD — прореживание подписей/делений на сильном отдалении (шаг 1/5/10/25/50 лет по `pxPerYear`), иначе 200 лет наезжали в кашу. Оба пофикшены отдельным коммитом после Ф4.
